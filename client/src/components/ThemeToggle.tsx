...
function StarsIcon({ className }: { className?: string }) {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={className}>
      ...
      <path d="M19 7L19.5 9L21.5 9.5L19.5 10L19 12L18.5 10L16.5 9.5L18.5 9L19 7Z" fill="currentColor"/>
...
export default function ThemeToggle({ theme, onToggle }: ThemeToggleProps) {
  return (
    <div className="bg-black/10 dark:bg-white/10 border border-black/20 dark:border-white/20 rounded-full px-3 py-2 backdrop-blur-sm">
      <button
        onClick={onToggle}
        className="relative w-8 h-8 flex items-center justify-center transition-all duration-200 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-black/20 dark:focus:ring-white/20 focus:ring-offset-2 rounded-full"
      >
        {/* Dark mode icon (stars) - shown in dark mode */}
        <StarsIcon 
          className={`absolute inset-0 w-6 h-6 text-white transition-all duration-300 ease-in-out ${
            theme === "dark" 
        ...
        <SunIcon 
          className={`absolute inset-0 w-6 h-6 text-gray-600 transition-all duration-300 ease-in-out ${
            theme === "light" 
              ? "scale-100 opacity-100" 
              : "scale-0 opacity-0"
          }`}
